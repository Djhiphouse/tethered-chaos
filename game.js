...
function freshState(){return{
  running:true,paused:false,time:0,wave:1,waveTime:0,score:0,combo:1,comboClock:0,charge:100,shake:0,flash:0,gameOver:false,modifier:null,
...
function simulate(dt,inputs){
  ...
  for(const p of state.players.filter(p=>p.down)){const mate=state.players.find(q=>q!==p);if(!mate.down&&dist(p,mate)<92){p.revive+=dt;floater(p.x,p.y-31,`${Math.min(100,Math.round(p.revive/2.2*100))}%`,p.color,12);if(p.revive>=2.2){p.down=false;p.hp=2;p.inv=2;p.revive=0;state.score+=500;ring(p.x,p.y,p.color,180);burst(p.x,p.y,p.color,35,300);floater(p.x,p.y,'RETTUNG +500',p.color,22);beep(660,.2,'sine',.06)}}else p.revive=Math.max(0,p.revive-dt*.45)}
